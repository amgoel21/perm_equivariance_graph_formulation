...
from GRIT.grit.layer.grit_layer import GritTransformerLayer  # Assuming GRITLayer exists in the GRIT repo
...
class GRITModel(nn.Module):
    def __init__(self, n_nodes, hid_dim, perms, coords_dim=(2, 3), num_layers=4, p_dropout=None, sparse=False, maxnorm=20, soft=False):
        ...
        self.grit_layers = nn.ModuleList([
            GritTransformerLayer(hid_dim, hid_dim, num_heads = 1, edge_dim=hid_dim) for _ in range(num_layers)
        ])
    ...
    def forward(self, x):
        ...
        for grit_layer, ln in zip(self.grit_layers, self.layer_norms):
            h_prev = h
            h = grit_layer(h, batched_edge_index, edge_features.reshape(-1, edge_features.size(-1)))
            h = ln(h)
            h = F.relu(h)
            h = h + h_prev
