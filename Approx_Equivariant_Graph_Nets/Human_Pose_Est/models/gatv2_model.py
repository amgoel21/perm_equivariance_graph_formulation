...
#         embeddings = embeddings * norms / torch.norm(embeddings, dim=1, keepdim=True)
#         return embeddings


# class GATv2Model(nn.Module):
#     """
#     GAT-based model with permutation-aware edge features
#     """
...
#         self.lin_edge = nn.Linear(hid_dim, hid_dim)  # Adjust dimensions as necessary
#         # GAT layers
...
#             conv = GATv2Conv(hid_dim, hid_dim, edge_dim=hid_dim, heads = 8)
...
#             # Create edge features using embeddings
#             edge_features = self.lin_edge(self.edge_embedder(self.edge_categories.to(device)))
#             # Initial feature transformation
#             h = self.input_mlp(item)
            
#             # Apply GATConv layers with residual connections and layer norm
...
#         # Use vmap to process the batch in parallel
...
class GATv2Model(nn.Module):
    ...
    """
    def __init__(self, n_nodes, hid_dim, perms, coords_dim=(2, 3), num_layers=4, p_dropout=None, sparse=False, maxnorm=100000000000000000, soft = False):
        super(GATv2Model, self).__init__()
        self.n = n_nodes  # number of nodes
        self.hid_dim = hid_dim
