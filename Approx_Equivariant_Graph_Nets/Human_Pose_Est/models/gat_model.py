...
#             num_heads = 8
#             conv = GATConv(hid_dim, hid_dim, edge_dim=hid_dim, heads = num_heads, concat=False)
...
#         for conv, ln in zip(self.conv_layers, self.layer_norms):
#             # Expand edge features for each batch graph
#             h_prev = h
...
class GATModel(nn.Module):
    ...
    """
    def __init__(self, n_nodes, hid_dim, perms, coords_dim=(2, 3), num_layers=4, p_dropout=None, sparse=False, maxnorm=20, soft = False):
        super(GATModel, self).__init__()
        self.n = n_nodes  # number of nodes
        self.hid_dim = hid_dim
        ...
        for _ in range(num_layers):
            conv = GATConv(hid_dim, self.head_dim, edge_dim=hid_dim, heads=self.num_heads, concat=True)
            conv_layers.append(conv)
            layer_norms.append(nn.LayerNorm(hid_dim))
