...
def create_datasets():
    ...
    structures = [ "palindrome","cyclicsum", "intersect", "set"]
    ...
    for structure_id in structures:
        ...
        if structure_id == "palindrome":
            seq_length = 12
            dataset = IsPalindromeDataset(num_samples=3500, seq_length=seq_length, palindrome_length=4,equivariant=True)
            ...
            seq_length = 12
            dataset = IntersectDataset(num_samples=3500, seq_length=seq_length, vocab_size=13,equivariant=True)
            graph_configs[structure_id] = {
            "n_nodes": seq_length,
            ...
            seq_length = 12
            dataset = MaxCyclicSumDataset(num_samples=3500, seq_length=seq_length, cyc_length = 5, vocab_size=13)
            graph_configs[structure_id] = {
            "n_nodes": seq_length,
...
def train_equiv():
    # Single trial with multiple datasets
    graph_configs, train_datasets, val_datasets, test_datasets = create_datasets()
    ...
    for trial in range(3):
        ...
        # Prepare training data (reshuffle every trial)
        all_train_data = []
        for struct in train_datasets:
            all_train_data.extend(train_datasets[struct])
        ...
        test_loaders = {
            struct: DataLoader(list(test_datasets[struct]), batch_size=64, shuffle=False, collate_fn=collate_fn)
            for struct in test_datasets
        }

        model = MultiGraphGATv2Model_equiv(graph_configs, hid_dim=128, num_layers=4, p_dropout=0.1, vocab_size=82).to(device)
        # optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
        gnn_params = []
        ...
        optimizer = torch.optim.Adam([
            {'params': gnn_params, 'lr': 0.003},            # GNN body
            {'params': edge_embedder_params, 'lr': 0.1},  # Edge embedders
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                for struct in val_loaders:
                    ...
                    avg_val_loss = total_val_loss / num_batches if num_batches > 0 else float('nan')
                    print(f"    Val Loss [{struct}]: {avg_val_loss:.4f}")

        # Test losses
        print(f"=== Trial {trial + 1} Test Losses ===")
        model.eval()
        with torch.no_grad():
...
def run_experiments():
    # Run multiple trials with various data settings
    import copy
